...
We have choosen to work with one of the simpler pretrained models, here the VGG19 model, but you can easily use different models from the Keras library as the procedure is the same. The VGG19 model is a CNN that is trained on more than one million images from ImageNet. The VGG19 model is 19 layers deep.
...
```{r}
vgg19 = keras::application_resnet50(include_top = FALSE,
                                       weights = 'imagenet', 
...
```{r}
resnet501 = vgg19
resnet501 %>% freeze_weights(from = 1, to = 174)
```
